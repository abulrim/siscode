...
(function ($, Backbone, _, H, amplify, undef) {
  ...
  InstitutionModel = Backbone.Model.extend();

  InstitutionCollection = Backbone.Collection.extend({
    model: InstitutionModel
  });

  InstitutionListView = Backbone.View.extend({
    ...
    fillSubjects: function() {
      ...
      MySubjectCollection.reset(selectedModel.subjects);
      this.parentView.institutionChanged(selectedModel);
    },
  ...
  CourseInputListView = Backbone.View.extend({
    ...
    addInstitutions: function() {
      var institutions = new InstitutionListView({ collection: MyInstitutionCollection });
      institutions.parentView = this;
      this.$('.cil-institutions').html(institutions.render().$el);
      institutions.afterRender();
    },
  ...
  MyInstitutionCollection = new InstitutionCollection();
  MyInstitutionCollection.reset(institutions);
